	...
	const changeSelectedStyleNo = (selectedStylesData: any) => {
		if (selectedStylesData) {
			...
			uniquemasterColorKeys.forEach((code) => {
				...
				const tempval_tempS = tempS > 0 ? tempS : 0;
				const tempval_tempM = tempM > 0 ? tempM : 0;
				const tempval_tempL = tempL > 0 ? tempL : 0;
				const tempval_tempXL = tempXL > 0 ? tempXL : 0;
				const tempval_temp2XL = temp2XL > 0 ? temp2XL : 0;

				const SETSIZE_XS = tempval_tempXS > 0 ? Math.round((Number(tempval_tempXS) / 100) * orderPrecentage) : 0;
				const SETSIZE_S = tempval_tempS > 0 ? Math.round((Number(tempval_tempS) / 100) * orderPrecentage) : 0;
				const SETSIZE_M = tempval_tempM > 0 ? Math.round((Number(tempval_tempM) / 100) * orderPrecentage) : 0;
				const SETSIZE_L = tempval_tempL > 0 ? Math.round((Number(tempval_tempL) / 100) * orderPrecentage) : 0;
				const SETSIZE_XL = tempval_tempXL > 0 ? Math.round((Number(tempval_tempXL) / 100) * orderPrecentage) : 0;
				const SETSIZE_2XL = tempval_temp2XL > 0 ? Math.round((Number(tempval_temp2XL) / 100) * orderPrecentage) : 0;
				...
				const lineToBeCreated = {
					...
					SETSIZE_XL,
					SETSIZE_2XL,
					TOTALQTY: SETSIZE_XS + SETSIZE_S + SETSIZE_M + SETSIZE_L + SETSIZE_XL + SETSIZE_2XL,
			...
			selectedStylesData.newLines.forEach((line) => {
				...
				//line.packMethod = 'Single pc packing-SIN';	
				line.wareHouse = '';
				line.requestDelDate = '';
	...
	return (
		<React.Fragment>
			<Grid container direction='row' justify='space-evenly'>
				...
				<Grid item xs={4}  hidden={true} style={{ paddingLeft: '0.5rem', paddingTop: '0.5rem' }}>
				...
				<Grid item xs={2} style={{ paddingLeft: '0.5rem', paddingTop: '0.5rem' }}>
				...
				<Grid item xs={3} style={{ paddingLeft: '0.5rem', paddingTop: '0.5rem' }}>
					<Button
						size={"medium"}
						variant='contained'
						color='primary'
...
export default Step2Componentv2;
